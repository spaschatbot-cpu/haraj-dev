...
### وقياسٌ ثانٍ — ١٣ سبتمبر ٢٠٢٦، بعد T830ج…ي و T866 و T868

**عمود v2 وحده تغيّر؛ عمود v1 كما هو** (نظامُ إنتاجٍ لا يُمَسّ، ولم يُعَد
استخراجه اليوم).

| | v1 (`admin_v2`) | v2 — ٠٩-٠٦ | v2 — ٠٩-١٣ |
|---|---|---|---|
| مسارات (GET + POST) | **٢٥١** | ٤٣ | **١٢٣** |
| متحكّمات / وحدات | **٣٢** | ١٥ | **٤٥** |
| قوالب عرض | **١٠٨** | ٣٤ | **٩٥** |
| صفحات في الشريط الجانبي | (لقطة المالك: ٦١) | ١٦ | **٥٥** |
| مداخلُ محجوزة في الشريط | — | ٤٨ | **١٢** |

كيف قِيست: المسارات بـ`get_resolver()` على البادئة `console/` لا بـ`grep`
(الـ`grep` يعدّ سطراً لا مساراً، ويُسقط ما يُضمَّن من وحدةٍ أخرى)؛ والشريطُ
**من الصفحة المرسومة** على `localhost:8001` — ٥٥ وسمَ `<a>` و١٢ وسمَ
`<span class="nav__soon">` في أحدَ عشرَ قسماً — لا من عدّ صفوف `PAGES`
(فيها ٥٨ صفّاً، ثلاثةٌ منها صفحاتُ تفصيلٍ بلا قسم فلا تظهر).

**والفجوةُ انقلبت في اتجاهٍ واحد ولم تُغلق:** ٤٥ وحدةً مقابل ٣٢ لا تعني تفوّقاً
— الوحدةُ هنا ملفٌّ لا متحكّم، وv1 يحشر عشر شاشاتٍ في ملفٍّ واحد. الرقمُ الذي
يُقاس عليه الإغلاقُ هو صفحاتُ الشريط: **٥٥ من ٦١**، والباقي في T832.
