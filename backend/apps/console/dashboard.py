...
def board_for(user) -> Board:
    ...
    # الفصلُ باقٍ والحقلُ باقٍ: لا بطاقةَ تحمل رسماً اليوم (أُزيلا بأمر
    # المالك)، فـ`charts` تخرج فارغةً و`stats` تأخذ الكلّ. والقاعدةُ تبقى
    # مكتوبةً لأن إعادةَ رسمٍ إلى بطاقةٍ يوماً لا ينبغي أن تُعيد كتابة الفرز.
