...
class Stat:
    ...
    unit: str = ""
    #: كتلةُ البطاقة في الرئيسية: `"money"` للصفّ الأعلى، وما عداه للحركة.
    #:
    #: **تُكتب على البطاقة لا تُستنتَج من لونها.** كان الفرزُ بالـ`tone`، وهو
    #: يعطي ثلاثاً وخمساً — والمالكُ أراد أربعاً وأربعاً، و«المزادات» فوق مع
    #: المال. واللونُ يقول «ما نوعُ هذا الرقم»، والكتلةُ تقول «أين يُقرأ»؛
    #: وهما سؤالان، وجعلُ أحدهما جواباً للآخر يجعل تغييرَ لونٍ ينقل بطاقة.
    #:
    #: **وفي آخر الحقول عمداً** — كلُّ نداءٍ في هذا الملفّ موضعيّ.
    group: str = "flow"
...
def board_for(user) -> Board:
    ...
    if sees_auctions:
        ...
        board.stats.append(
            Stat(
                ...
                "award",
                "افتح المزادات",
                # **المزاداتُ في كتلة المال** بأمر المالك: هي المخزونُ الذي
                # يقابل تلك الأرقام — رصيدٌ وحجزٌ وفاتورةٌ ومزادٌ تُقرأ معاً،
                # وما يقع عليها (مزايدةٌ ورفضٌ وقرار) يُقرأ تحتها.
                group="money",
                unit="مزاد",
            )
        )
        ...
        board.stats.append(
            Stat(
                ...
                # **بلا وجهةٍ ثانيةً** (T939): كانت تفتح «تحليل المزايدات»
                # وقد حُذفت بطلب المالك. و«مزايدات السيارات» ليست بديلاً —
                # تلك جدولُ صفوفٍ يُبحَث فيه، وهذه تقرأ رقماً كلّيّاً. ورابطٌ
                # إلى شاشةٍ لا تجيب سؤالَ البطاقة أسوأُ من لا رابط.
                "",
                "auction",
                "gavel",
                "",
    ...
    #
    # **والقاعدةُ مكتوبةٌ على البطاقة (`group`) لا مستنتَجةٌ من لونها.** كان
    # الفرزُ بالـ`tone` فأعطى ثلاثاً وخمساً؛ والمالكُ أراد أربعاً وأربعاً
    # و«المزادات» فوق. واللونُ يقول «ما نوعُ هذا الرقم» والكتلةُ تقول «أين
    # يُقرأ» — سؤالان، وجعلُ أحدهما جواباً للآخر يجعل تغييرَ لونٍ ينقل بطاقة.
    #
    # وعنوانٌ بلا قاعدةٍ تحته يصير سطراً يفصل حيث انتهى صفٌّ لا حيث انتهى
    # معنى: تُضاف بطاقةٌ يوماً فيقع العنوانُ في منتصف موضوعٍ واحد.
    board.charts = [s for s in board.stats if s.spark]
    board.stats = [s for s in board.stats if not s.spark]
    board.flow = [s for s in board.stats if s.group != "money"]
    board.stats = [s for s in board.stats if s.group == "money"]
