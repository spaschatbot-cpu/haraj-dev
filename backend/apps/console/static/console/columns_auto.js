...
(function () {
  ...
  function setup(table, index) {
    ...
    /* **إلّا عموداً يقول عن نفسه إنه ليس هويّة.** T966.
    
       القفلُ على الأوّل حجّتُه أنه هويّةُ الصفّ. وهي صحيحةٌ في كلّ جدولٍ في
       اللوحة إلّا واحداً: «اتخاذ القرار» تعرض أعمدةَ v1 بترتيبه، وأوّلُها
       **«معرف المركبة»** — مفتاحُ قاعدةِ بياناتٍ داخليّ، أقلُّ ما يُقرأ
       وأوّلُ ما يُستغنى عنه. وهويّةُ الصفّ هناك «موقف المركبة» بعده.
    
       فبقاؤه مقفولاً كان يعني أن الجدولَ يفيض ولا شيءَ يمكن إخفاؤه. و
       `data-drop` يرفع القفلَ عنه وحدَه: يُخفى تلقائيّاً إن لزم، ويظهر في
       المنتقي ليعود بضغطة. */
    for (var d = 0; d < heads.length; d++) {
      if (heads[d].hasAttribute("data-drop")) delete locked[keys[d]];
    }
