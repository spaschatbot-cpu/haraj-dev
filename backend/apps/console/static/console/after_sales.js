 ...
 *
 * **البيانات من الصفّ لا من الخادم.** كُتب هنا أن v1 يفتح `<iframe>` من
 * `/invoice?inv=…` — **ولا وجود لذلك المسار**؛ زرُّ «عرض» عنده
 * `<a href="invoice_view.php?id=…" target="_blank">` (تبويبٌ جديد بمستند HTML
 * كامل + Bootstrap) لكل ضغطة. والحقولُ كلُّها مرسومةٌ أصلاً في الصفّ، فتُقرأ
 * من `data-*` بلا رحلةِ شبكةٍ ولا انتظار.
 *
...
(function () {
  ...
  /* نبرةُ حبّة الحالة تأتي مع الصفّ في `data-tone` — محسوبةً في بايثون من
     `tones.py`، الخريطةِ الواحدة.

     وكانت هنا خريطةٌ ثالثة مفتاحُها **الاسمُ العربيّ** («مسدَّدة» ← ok)،
     فأخطأت حيث أخطأ القالب: «مستحقة» بلا نغمة، وهي `warn` في الخريطة
     الواحدة. وخريطةٌ بالاسم المعروض تنكسر صامتةً يومَ تُعدَّل كلمةٌ في
     `InvoiceState.choices` — تبقى الحبّة رماديّةً ولا يُخطئ شيء. */
  ...
  function fill(dialog, source) {
    ...
    if (pill) pill.setAttribute("data-tone", source.dataset.tone || "");

    /* كان هنا سطرٌ يملأ `[data-inv-href]` برابط صفحة الفاتورة — **ولا عنصرَ
       بهذا الاسم في السند**، فكان `querySelector` يرجع `null` ويمضي بلا أثر
       ولا خطأ. حُذف هو و`data-href` معاً: لا تُفتح صفحةٌ من هذه الشاشة إلا
       السند. */
  ...
  /* الطباعة عبر `<iframe>` مستقلٍّ من صنعنا — لا طباعةَ الصفحة نفسها.
