...
PAGES: tuple[Page, ...] = (
    ...
    # **«ما بعد البيع» لم تعد صفّاً في الشريط** — T973. دُمجت في «القرارات
    # المنتهية» بعد مسحِ الأربعين شاشةً بحثاً عن متشابهات: هي الزوجُ الوحيدُ
    # الذي ظهر (٩٤٪ — خمسةَ عشرَ عموداً مشتركاً من ستّةَ عشر، وبانيا الصفوف
    # واحد، والقالبُ واحدٌ بالوراثة).
    #
    # وقِيس صفّاً بصفّ أنها ≡ «القرارات المنتهية `?which=awarded`»: اثنا عشرَ
    # صفّاً في الاثنتين، صفرٌ زائدة وصفرٌ ناقصة.
    #
    # **وبقيت تلك لا هذه لأن الاسمَ يجب أن يصدق**: «ما بعد البيع» لا تصدق على
    # مركبةٍ رفضها المالك — لم يقع بيعٌ فلا «بعد» له. ومسارُها باقٍ تحويلاً
    # يحمل `which=awarded` (`after_sales.after_sales`).
    Page(
    ...
    # **والصفُّ يبقى بقسمٍ فارغ، ولا يُحذف.** `console_page` يقرأ القدرةَ من
    # هذا السجلّ، فصفٌّ محذوفٌ يعني تحويلاً **بلا حارسٍ ولا تسجيلِ دخول**.
    # والقسمُ الفارغ يُخرجه من الشريط ومن شبكة الرئيسية ويُبقي حارسَه — وهو
    # النمطُ نفسُه المستعمَل في «إنشاء دفعة» و«التحكم في صفحات المستخدمين».
    Page(
        "console:after-sales",
        "ما بعد البيع",
        Capability.AUCTIONS_VIEW,
        "",
        parent="console:ended-decisions",
    ),
    Page(
        ...
        Capability.AUCTIONS_VIEW,
        "auctions",
        "ما حُسم فيه قرار — ومرشِّحُ «رست» يعطي ما بعد البيع وحدَه.",
        "stamp",
    ),
    ...
    Page(
        ...
        # **قسمُ المزايدات لا «النظام والصلاحيات».** T969
        #
        # كان فوق الشاشة ثلاثةُ كروتٍ تقول تسلسلَ العمل: المزادُ الجاري ←
        # اتخاذ القرار ← أرشيفُ المنتهية (نقلاً عن `nav-cards` في v1). وقرارُ
        # المالك (٢٢ سبتمبر ٢٠٢٦): «اعمل التلاتة دول في السايد بار» — أي أن
        # التسلسلَ مكانُه الشريط، لا صفٌّ يتكرّر فوق كلّ شاشةٍ منها.
        #
        # وكانت هنا في «النظام والصلاحيات» يومَ كانت «منصّة الملّاك» سبعةَ
        # أرقام. وهي اليوم شاشةُ قرارٍ بين شاشتَي قرارٍ أُختَين، فمكانُها
        # بينهما — ويُقرأ التسلسلُ في الشريط كما كان يُقرأ في الكروت.
        "decisions",
    ...
    # **«ملخّص المقبولة» لم تعد صفّاً في الشريط** — T972. دُمجت في
    # «المزايدات المقبولة» بسؤال المالك: «لو شبه بعض ادمجهم». وكانت شاشةً
    # تستدعي الاستعلامَ نفسَه بالمرشّحات نفسها وتعرض الجدولَ نفسَه، وفرقُها
    # ستُّ بطاقاتٍ — صارت فوق الجدول هناك.
    #
    # ومسارُها باقٍ تحويلاً (`decisions.accepted_summary`) لمن يحمل رابطاً
    # قديماً، **ولا صفَّ له هنا**: صفٌّ يقود إلى تحويلٍ يُقرأ شاشتين.
    # **والصفُّ يبقى بقسمٍ فارغ**: `console_page` يقرأ القدرةَ من هذا السجلّ،
    # فصفٌّ محذوفٌ يعني تحويلاً بلا حارسٍ ولا تسجيلِ دخول.
    Page(
        "console:accepted-summary",
        "ملخّص المقبولة",
        Capability.AUCTIONS_VIEW,
        "",
        parent="console:accepted-bids",
    ),
    Page(
    ...
    # **وعاد «إيداع يدوي» وحده** (قرار المالك، ٢٤ سبتمبر ٢٠٢٦): اختبارُ المسار
    # الكامل وقف عند عميلٍ لا يستطيع إيداعَ تأمينه إلا ببطاقة، ولا بابَ لمالٍ
    # وصل نقداً أو بحوالة. والخصمُ المباشرُ وطلباتُ الشحن البنكيّ باقيةٌ ملغاة.
    Page(
        "console:wallet-credit",
        "إيداع يدوي",
        Capability.MONEY_ACT,
        "wallet",
        "إيداعُ تأمينٍ بمرجعٍ وسبب — وديعةٌ كاملة، والمرجع يمنع التقييد مرّتين.",
        "plus-wallet",
    ),
    ...
    Page(
        ...
        # **«اتخاذ القرار للشريك» — والاسمُ حُسم بعد جولتين.** T968
        #
        # سمّتها T966 «قرارات الشريك» لأن «اتخاذ القرار» انتقل إلى
        # `console:owners-console` بقرار المالك في ٢١ سبتمبر. ثمّ رُدَّ الاسمُ
        # إليها في اليوم التالي بقرارٍ آخر — **فصار الاسمُ على صفّين معاً**،
        # وهو ما حذّرت منه T966 بحقّ: من يبحث عن شاشته يجد اثنتين ولا يفرّق
        # بينهما إلّا القسم. قِيس بعدّ عناوين `PAGES`: تكرارٌ واحدٌ في الشريط
        # كلِّه، هذا.
        #
        # فحسمه المالك في ٢٢ سبتمبر ٢٠٢٦: «خلي التانية اتخاذ القرار للشريك».
        # فالاسمان يفترقان بكلمةٍ واحدةٍ تقول الفرقَ الحقيقيّ: **هذه مُرشَّحةٌ
        # بالشريك** (`?partner=`) وتمتدّ على المزادات المنتهية كلِّها، وتلك
        # مزادٌ واحدٌ بعينه — الأخيرُ أو ما يُختار.
        #
        # ولا يُعاد فتحُ هذا: قُرئ الاسمُ ثلاثَ مرّاتٍ وحُسم في الثالثة.
        "اتخاذ القرار للشريك",
        Capability.PARTNERS_DECIDE,
        "partner",
