...
PAGES: tuple[Page, ...] = (
    ...
    #
    # و«حالة فاتورة» **باقيةٌ في `DETAIL_PAGES` لا محذوفة**: بحثُها بالشاصي
    # واللوحة صار مدخلاً في حقل البحث هنا، فالمالكُ يرى مدخلاً واحداً. لكنّ
    # قدرتَها `invoices.lookup` أضيقُ من `invoices.view` عمداً — تُجيب جهةً
    # **خارج الشركة** عن مركبةٍ بلا مشترٍ ولا جوّالٍ ولا مبلغ — وحذفُها كان
    # يعني منحَ من يجيب هاتفَ التأمين قراءةَ القائمة كلِّها بمبالغها.
    Page(
        "console:invoices",
        "مركز الفواتير",
        Capability.INVOICES_VIEW,
...
DETAIL_PAGES: tuple[Page, ...] = (
    ...
    # خرجت من الشريط في T936 وبقيت صفحةً: السببُ فوق عند صفّ «مركز الفواتير».
    Page(
