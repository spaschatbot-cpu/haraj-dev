    ...
    **والعنوان «حسابات» لا «عملاء».** v1 يقول «إجمالي العملاء» وهو يعدّ جدولاً
    ...
    return [
        ...
        Stat(
            ...
            # السطرُ يقابل سطرَ الشركات: كلاهما يقول **بمن تُنسب المزايدة**،
            # وهو الفرقُ العمليُّ الوحيد بين النوعين في هذه الشاشة. وبطاقةٌ
            # بلا سطرٍ بين أربعٍ لها سطور تُقرأ ناقصةً لا مختصرة.
            detail="تزايد باسم صاحبها ورقم هويته.",
            icon="person-search",
        ),
    ]
...
def staff_grants(request, pk: int):
    ...
    return render(
        ...
        "console/staff_grants.html",
        {
            ...
            "effective": effective,
            "grants": grants_qs,
            "granted_count": grants_qs.filter(granted=True).count(),
            "revoked_count": grants_qs.filter(granted=False).count(),
            "is_owner": is_owner_account(member),
            "role_label": role_label(member),
            "crown_icon": path_of("crown"),
            "shield_icon": path_of("shield"),
            "check_icon": path_of("check"),
            "ban_icon": path_of("ban"),
        },
    )
