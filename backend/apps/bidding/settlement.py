...
def pay_vehicle_invoice_from_balance(
    *,
    ...
    ⚠️ **ولا بابَ لهذه الدالّة من التطبيق بعد اليوم.** T954.

    قرارُ المالك (١٩ سبتمبر ٢٠٢٦): «رصيد التأمين لا يمكن وممنوع السداد منه
    للفواتير» — وهي المادةُ السادسة: «مبلغ الضمان لا يُحتسب من ثمن المركبة».
    فأُغلقت `InvoicePayView` وصارت ترفض بجملةٍ عربيّة، ولم يبقَ لهذه الدالّة
    مُنادٍ إلا `seed_demo` — وهو لا يعمل خارج `DEBUG`.

    وبقيت لأن بيانات العرض تحتاج فواتيرَ مسدَّدةً بمسارٍ واحد. والفاتورةُ
    الحقيقيّة تُسدَّد بحوالةٍ بنكيّةٍ يسجّلها `money.record_payment` عند
    تأكيد أودو.
    """
