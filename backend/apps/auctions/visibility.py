...
def current_auction_ids(*, now=None) -> list[int]:
    """المزادُ «الحاليّ» — بقاعدة v1 حرفاً. T952.

    ## القاعدة، ونصُّها من v1 نفسِه

    `ajax_my_bids.php` يحملها في تعليقٍ عربيٍّ فوق استعلامها:

        مشاركاتي = المزاد «الحالي» للعميل فقط:
        • لو فيه مزاد نشط → يعرض النشط فقط، فبمجرد ما المزاد الجديد يبقى
          نشط تختفي مشاركات المزاد اللي قبله فوراً.
        • لو مفيش أي مزاد نشط (الفترة بين انتهاء مزاد وبدء التالي) → يعرض
          آخر مزاد بدأ فعلاً (أحدث منتهي) فتفضل نتيجته ظاهرة لحد ما المزاد
          الجديد يبدأ.

    وطلبُ المالك (١٩ سبتمبر ٢٠٢٦): «شوفها في النظام القديم، طريقة العرض إنه
    آخر مزاد بس اللي يظهر، ولو في واحد نشط تظهر مزايداته هو بس».

    ## ولماذا قاعدةٌ لا ترتيبٌ بالتاريخ

    «مشاركاتي» ليست سجلَّ مزايداتٍ أبديّاً، هي **ما يخصّ العميلَ الآن**: ماذا
    وضعتُ في المزاد الجاري، أو — بين مزادين — ماذا كانت نتيجتي في الذي انتهى.
    وقائمةٌ مرتّبةٌ بالتاريخ تخلط الاثنين، فيقرأ العميلُ مزايدةً من مزادٍ
    انتهى قبل شهرين على أنها قائمة.

    ## وفرقان مقصودان عن v1

    * **المسودّةُ والملغيُّ يُستثنيان من البديل.** v1 يأخذ «آخر ما بدأ» بلا
      شرطٍ على الحال، فمزادٌ ألغي بعد بدئه يصير هو «الحاليّ» وتختفي نتيجةُ
      المزاد الحقيقيّ قبله. وهذا عطبٌ لا قاعدة.
    * **الوقتُ UTC** ولا `CONVERT_TZ`: v1 يخزّن بتوقيت الرياض فيحوّل في كلّ
      استعلام، ونحن نخزّن UTC (المادة ٣-١).

    و**قائمةٌ لا معرّفٌ واحد**: v1 يستعمل `IN (…)`، فمزادان جاريان معاً
    يظهران معاً. وذلك صحيح — الجاري هو الجاري، ولو كانا اثنين.
    """
    ...
    started = Auction.objects.filter(starts_at__lte=now).exclude(
        state__in=(AuctionState.DRAFT, AuctionState.CANCELLED)
    )

    live = list(
        started.filter(state=AuctionState.LIVE)
    ...
    # البديل: آخرُ ما بدأ فعلاً — بنهايته ثمّ بمعرّفه، كما يرتّب v1
    # (`ORDER BY a4.end_time DESC, a4.id DESC`).
    latest = started.order_by("-ends_at", "-pk").values_list("pk", flat=True).first()
