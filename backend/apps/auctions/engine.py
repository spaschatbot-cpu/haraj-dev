...
class Badge(models.TextChoices):
    """ما يظهر في خانة «الحالة» — خمسُ كلماتٍ، **ثلاثٌ تُحسب من الموعدين**.
    ...
    **ثلاثاً محسوبة**: الأولَيان **لافتةُ عرضٍ** على العميل لا حالةَ مزاد —
    تبقيان في `Auction.showcase` وتُحرَّران من نافذة الحالة، ولا تُخلطان بما
    يقوله الموعدان.

    ومعهنّ **قراران يُقرآن من العمود** لا من الساعة — `draft` و`cancelled`،
    كما يقول `badge_of` تحت — فالتعدادُ **خمسةُ أعضاء**. وكان مكتوباً هنا
    «صارت أربعاً» و«أربعُ كلماتٍ تُحسب من الموعدين» والأعضاءُ خمسةٌ ولا
    يُحسب منها إلا ثلاث: نصٌّ يعدّ غيرَ ما في الكود يجعل قارئَه يبحث عن عضوٍ
    زائدٍ لا وجود له. **النصُّ صُحّح، والكودُ لم يُمسّ.**
    """
...
class RowSummary:
    ...
    #: مركباتٌ في هذا المزاد تنتظر قراراً. T860
    #:
    #: هذا هو ما تجيبه نافذةُ «مراجعة العروض» في v1: أيُّ سيارةٍ في هذا المزاد
    #: عليها عرضٌ لم يُقبَل ولم يُرفَض بعد. وv1 يفتح النافذة ثم يقول «لا سيارات»
    #: — فيدفع الموظّف نقرةً ليعرف أن لا شيء ينتظره. والعددُ هنا في الصفّ، فلا
    #: يُنقر إلا ما فيه شيء.
    undecided: int = 0
...
def summarise(auctions) -> dict[int, RowSummary]:
    ...
    # ٤ — ما ينتظر قراراً. استعلامٌ مجمَّعٌ كالبقيّة، لا واحدٌ لكل صفّ. T860
    #
    # و`AWAITING_DECISION` وحدها لا `DECIDABLE` كلُّها: السؤال الذي يجيبه
    # العددُ على الصفّ هو «هل عليّ شيءٌ الآن»، ومركبةٌ رست بالفعل ليست عليك —
    # هي قابلةٌ لنقل الفائز إن لزم، وذاك بابٌ آخر لا تنبيهٌ يوميّ.
    for row in (
        Vehicle.objects.filter(
            auction_id__in=ids, state=VehicleState.AWAITING_DECISION
        )
        .values("auction_id")
        .annotate(undecided=Count("id"))
        .order_by()
    ):
        cars[row.pop("auction_id")].update(row)

    # ٥ — أشهرُ ماركتين، لأن «١٩٢ ماركة» رقمٌ لا يصف مزاداً. استعلامٌ خامس
