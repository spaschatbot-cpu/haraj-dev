...
class VehicleCardSerializer(serializers.Serializer):
    ...
    #: هل حفظ **هذا القارئ** المركبة في مفضّلته.
    #:
    #: الحقل الوحيد في الكرت الذي يخصّ القارئ لا المركبة، ولذلك يُحسب للصفحة
    #: كلها بطلبٍ واحد في `cards.vehicle_cards` لا لكل صفّ. وزائرٌ بلا جلسة
    #: يقرأ `false` دائماً — لا مفضّلة لمن لا حساب له، والسؤال ليس خطأً.
