...
class BankTopupState(models.TextChoices):
    ...
    # «اعتُمد وقُيِّد» لا «اعتُمد وأُضيف للرصيد»: الثانيةُ كانت صادقةً حين كان
    # المعتمَدُ هو المُدَّعى دائماً، وصارت كاذبةً حين صار الموظّف يعتمد ما وصل.
    # مبلغٌ ليس مضاعفاً للوديعة يُقيَّد في «المعلّق» ولا يصل رصيدَ العميل
    # (HR-03) — وقِيس: طلبٌ اعتُمد بـ٩٬٨٥٠ فقُرئ في الطابور «اعتُمد وأُضيف
    # للرصيد» ورصيدُ صاحبه صفر. والصفحةُ تقول أين وقع لكلّ صفٍّ على حدة.
    POSTED = "posted", "اعتُمد وقُيِّد"
    REJECTED = "rejected", "مرفوض"
    CANCELLED = "cancelled", "ألغاه العميل"
...
class BankTopupRequest(models.Model):
    ...
    ويُرسَل مسودّةً إلى أودو. ثم **يُعتمد من اللوحة بما وصل فعلاً** — لا اعتماداً
    متفائلاً كـ v1 الذي كان يطارد ما لم يصل البنك.

    `reference` الفريد هو المفتاح من الطلب إلى أودو والعودة: به يُقيَّد الائتمان
    مرّةً واحدة، فإعادةُ إرسالٍ لا تشحن مرّتين — وهو نفسُه مفتاحُ تفرُّد القيد
    (`deposit_key`)، فاعتمادٌ من اللوحة ثم تأكيدٌ من أودو على المرجع نفسِه
    يقيّدان **مرّةً واحدة** لا مرّتين.

    والاعتمادُ من اللوحة، لا من أودو وحده (تصحيحُ قرار ٢٠٢٦-٠٩-٠٨)
    ==============================================================
    كان مكتوباً هنا أن «اللوحة تقرأ ولا تعتمد». والقياسُ من دَمب الإنتاج يقول
    إن ذلك يترك الموظّفَ بلا أداة: ٢٣٢ طلباً كلُّها `wallet_topup` بتحويلٍ
    بنكيّ، و٢١ منها ما زالت `pending` — تنتظر يداً. والبابُ الوحيد إلى الدفتر
    يبقى واحداً على كل حال: :mod:`apps.money.services` هي كاتبُ الأرصدة، سواءٌ
    ناداها ويبهوك أودو أم زرُّ الموظّف.
    """
    ...
    #: ما **ادّعاه** العميل أنه حوّله. دعوى لا إثبات — انظر
    #: :attr:`amount_approved` تحته.
    amount = models.DecimalField(**MONEY)
    reference = models.CharField(max_length=64, unique=True)

    #: ما **وصل فعلاً** كما قرأه الموظّف في كشف حساب المنصّة. T919.
    #:
    #: `NULL` حتى يُراجَع الطلب، ثم رقمٌ لا يُشتقّ من :attr:`amount` بل يُكتب
    #: بيد من فتح الكشف. والفرقُ بينهما هو كلُّ سببِ وجوده: العميل يكتب
    #: «حوّلتُ ٥٬٠٠٠» والإيصالُ يقول ٤٬٨٥٠، فبلا هذا العمود أمام الموظّف بابان
    #: لا ثالثَ لهما — يقيّد الدعوى فيُضاف إلى المحفظة مالٌ لم يصل، أو يرفض
    #: الطلبَ كلَّه فيضيع تحويلٌ حقيقيّ ويعود العميل يشكو.
    #:
    #: وv1 يحمل العمودَ نفسَه (`transfer_requests.amount_approved`) ولا يكتبه
    #: إلّا **ويبهوك أودو**، بالمبلغ الذي رآه أودو نفسُه: ٢٠٥ من ٢٣٢ صفّاً
    #: معتمَدةٌ، وفي ٢٠٥ منها `amount_approved == amount` بلا استثناء — أي أن
    #: الموظّف هناك لا يملك أن يعتمد مخالفاً أصلاً، والفرقُ حين يقع يُحسم خارج
    #: النظام. وهنا يُكتب في اللوحة وله سببٌ إجباريّ حين يختلف.
    amount_approved = models.DecimalField(**MONEY, null=True, blank=True)

    #: سندُ المطابقة — الثلاثةُ التي بها يتحقّق الموظّف أصلاً. T919.
    #:
    #: التحويلُ البنكيّ لا يُخطِر أحداً: الموظّف يفتح كشفَ حساب المنصّة ويبحث
    #: عن سطرٍ بمبلغٍ وتاريخٍ واسمِ مُرسِل. فبلا اسمِ المُرسِل وتاريخِ التحويل
    #: لا سبيل إلى المطابقة إلّا بالصورة — وصورةٌ يرفعها العميل ليست إثباتاً،
    #: تُزوَّر بدقيقة.
    #:
    #: **وتُكتب هنا عند المراجعة لا عند الرفع**، وهو الفرقُ عن v1: أعمدتُه
    #: الأربعة (`bank_sender_name` · `bank_transfer_date` · `bank_iban` ·
    #: `bank_account_number`) يملؤها **العميل** في استمارة نقل الملكية، ومسارُ
    #: شحن المحفظة (`WalletTopupApiController`) لا يكتب فيها شيئاً — فهي
    #: **فارغةٌ في ٢٣٢ صفّاً من ٢٣٢** في نسخة الإنتاج. أي أن v1 لم يفقد
    #: الأعمدة، فقد فقد **من يملؤها**: ما يكتبه العميل دعوى ثانية بجوار الأولى،
    #: والمطابقةُ لا تقوم إلّا بما يقرؤه الموظّف من الكشف.
    bank_sender_name = models.CharField(max_length=120, blank=True)
    bank_transfer_date = models.DateField(null=True, blank=True)

    #: آيبانُ المُرسِل — **عمودٌ واحد لا عمودان**، خلافاً لـv1.
    #:
    #: v1 يحمل `bank_iban` و`bank_account_number` معاً لأن استمارتَه تسأل
    #: العميلَ الاثنين ويكتب ما كتب. وعملُ هذا الحقل هنا واحد: أن يُميّز الحساب
    #: المُرسِل في كشفٍ بنكيّ. والآيبان السعوديّ (`SA` + ٢٢) يحمل رقمَ الحساب
    #: داخله ويحمل معه المصرف، ورقمُ حسابٍ عارٍ بلا اسم مصرفٍ لا يُطابَق به
    #: سطرٌ في كشف. فعمودان أحدهما يُغني عن الآخر عمودان يتفارقان: يُملأ هذا
    #: مرّةً وذاك مرّةً، ثم يُسأل «أيُّهما الصحيح؟» — وهو سؤالٌ بلا جواب.
    #:
    #: ٣٤ محرفاً لأنه أطول آيبان في المعيار (لا ٢٤ السعوديّ وحده): العميلُ قد
    #: يحوّل من حسابٍ خليجيّ، وحقلٌ يقصّ الآيبان يُنتج مطابقةً كاذبة.
    ...
    class Meta:
        constraints = [
            ...
            # المعتمَدُ موجبٌ أو غير مكتوبٍ بعد — ولا ثالث. صفرٌ معتمَدٌ يقرأ
            # «رُوجع ووصل لا شيء»، وهو رفضٌ يلبس ثوبَ اعتماد.
            models.CheckConstraint(
                condition=Q(amount_approved__isnull=True) | Q(amount_approved__gt=ZERO),
                name="an_approved_bank_topup_amount_is_positive",
            ),
            # «اعتُمد» يسمّي كم اعتُمد. بلا هذا القيد يبقى الصفُّ `posted`
            # و`amount_approved` فارغاً، فيُقرأ بعد شهرٍ على أن المعتمَد هو
            # المُدَّعى — وهو بالضبط ما لا نعرفه حينها.
            models.CheckConstraint(
                condition=~Q(state="posted") | Q(amount_approved__isnull=False),
...
class PaymentSheet(models.Model):
    """دفعةٌ وصلت في ملفّ — وبصمتُها هي ما يمنع رفعها مرّتين. T830ي.
