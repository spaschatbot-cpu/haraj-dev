...
# عنوانُ أودو لبناء **روابط** الفواتير الضريبيّة في اللوحة فقط — لا استدعاءات:
# `apps.odoo.client.call` يظلّ محجوباً بـ`ODOO_ENABLED=False` (المادة ٢-٦)،
# فهذا يُظهر زرَّ «الفاتورة الضريبيّة» في «ما بعد البيع» ليُجرَّب دون أن يمسّ
# نظاماً محاسبياً. القيمةُ هي احتياطيُّ v1 نفسه.
# مباشرةً لا عبر `env`: ملفُّ `.env` هنا يحمل `ODOO_BASE_URL` فارغاً، فـ`env`
# يُرجعه فارغاً لا الافتراضيَّ. وهذا رابطٌ للعرض فقط، والاستدعاءُ محجوب.
ODOO_BASE_URL = "https://haraj1.odoo.com"
