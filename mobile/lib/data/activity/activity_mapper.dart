...
extension PurchaseMapper on api.Purchase {
  Purchase toDomain() => Purchase(
    ...
    stateLabel: '',
    // **الفاتورة على المشترى كاملةٌ لا ملخَّص.** كان هنا `null` وتعليلُه أن
    // العقد يرسل «خريطةً فيها معرّفها ورقمها» — وذلك وصفُ مُسلسِلٍ آخر
    // (`HoldSerializer.get_invoice`)؛ أمّا `PurchaseSerializer.get_invoice`
    // فيُرجع `InvoiceSerializer` بحذافيره: المبلغ والمسدَّد والمتبقّي والحالة
    // ووقت الإصدار.
    //
    // وثمنُ تلك الـ`null` لم يكن عرضاً ناقصاً بل **عجزاً عن الدفع**: شاشةُ
    // المشتريات تختار مركبةً وتضغط «دفع»، فتجد `invoice == null` فتقول «لا
    // فاتورة بعد» — وللمركبة فاتورةٌ مستحقّة. قِيس على جهاز في ١٦ سبتمبر
    // ٢٠٢٦: الممرُّ يُرجع `invoice.id = 73867` والتطبيق يراها معدومة.
    invoice: invoice == null ? null : api.Invoice.fromJson(invoice!).toDomain(),
  );
