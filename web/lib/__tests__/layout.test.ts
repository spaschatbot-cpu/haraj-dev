...
//: **وفرغت مع T1032**، وهذا هو الخبر: صار `PageShell` نفسه استجابياً — حشوةٌ
//: تتّسع مع المقاس، وتنقّلٌ يظهر أفقياً فوق `lg` ويصير شريطاً ممرَّراً دونه —
//: وهو في **كل** شاشة. فما من شاشةٍ تخطيطها واحدٌ على المقاسات الثلاثة بعد
//: اليوم، ولو كان محتواها هو هو.
//:
//: وتُترك القائمة موجودةً فارغةً لا تُحذف: الاختبار الذي يقرؤها هو ما يجعل
//: **دخول** شاشةٍ إليها قراراً يُراجَع — ولو حُذفت لصار «هذه الشاشة لا تستجيب
//: للمقاس» شيئاً يمرّ بصمت.
const SIZE_INVARIANT: string[] = [];
...
describe("J9 — الشاشة على مقاس جوال", () => {
  ...
  it("ولا بادئة صنف يجهلها الفاضّ — وإلا اختفى الصنف من اللقطة بلا أثر", async () => {
    ...
    //: `placeholder` و`focus` أُضيفا مع نظام التصميم (T1032): حالتان لا
    //: تغيّران تخطيطاً — كـ`hover` تماماً — لكن تركهما مجهولَين يعني اختفاء
    //: الصنف من إسقاط المقاس بلا أثر، وهو ما يحرسه هذا الاختبار بعينه.
    const known = new Set([
  ...
  it("لا عرض ثابت بالبكسل في أي شاشة", async () => {
    ...
    for (const root of ["app", "features"]) {
      for await (const path of walk(root, [".tsx", ".css"])) {
        ...
        //
        //: و`(?<!max-)` على **الصنف** أيضاً لا على خاصّية CSS وحدها: `\b`
        //: تطابق بعد الشرطة في `max-w-[1380px]`، فكان الفحص يرفض حدّاً أعلى —
        //: وهو الشكل الذي تُكتب به هذه التخطيطات، وقاله السطران أعلاه ولم
        //: يفعله النمط. ظهر يوم صار للشاشة سقفُ عرض (T1032).
        if (
          /(?<!max-)\bw-\[\d{3,}px\]|(?<!max-)\bmin-width:\s*\d{3,}px|(?<!max-)\bwidth:\s*\d{3,}px/.test(
            source,
          )
        ) {
          offenders.push(path);
